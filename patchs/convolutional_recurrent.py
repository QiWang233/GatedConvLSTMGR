...
class ConvRNN2D(RNN):
  ...
  """
    ...
    def __init__(self,
                 ...
                 unroll=False,
                 **kwargs):
        print("ConvRNN2D __init__ occur!")
        if unroll:
    ...
    def compute_output_shape(self, input_shape):
        print("ConvRNN2D compute_output_shape occur!")
    ...
    def get_initial_state(self, inputs):
        print("ConvRNN2D get_initial_state occur!")
    ...
    def __call__(self, inputs, initial_state=None, constants=None, **kwargs):
        print("ConvRNN2D __call__ occur!")
    ...
    def reset_states(self, states=None):
        print("ConvRNN2D reset_states occur!")
...
class ConvLSTM2DCell(Layer):
  ...
  """
    ...
    def __init__(self,
                 ...
                 recurrent_dropout=0.,
                 **kwargs):
        print("ConvLSTM2DCell init occur")
    ...
    def build(self, input_shape):
        print("ConvLSTM2DCell build occur")
...
class GatedConvRNN2D(RNN):
  ...
  """
    ...
    def __init__(self,
                 ...
                 unroll=False,
                 **kwargs):
        print("GatedConvRNN2D __init__ occur!")
        if unroll:
            raise TypeError('Unrolling isn\'t possible with '
                            'convolutional RNNs.')
        if isinstance(cell, (list, tuple)):
            ...
            raise TypeError('It is not possible at the moment to'
                            'stack convolutional cells.')
        super(GatedConvRNN2D, self).__init__(cell,
                                             return_sequences,
                                             return_state,
                                             go_backwards,
                                             stateful,
    ...
    @tf_utils.shape_type_conversion
    def compute_output_shape(self, input_shape):
        print("GatedConvRNN2D compute_output_shape occur!")
        if isinstance(input_shape, list):
            input_shape = input_shape[0]
    ...
    @tf_utils.shape_type_conversion
    def build(self, input_shape):
        print("GatedConvRNN2D build occur!")
        # Note input_shape will be list of shapes of initial states and
        # constants if these are passed in __call__.
        if self._num_constants is not None:
            constants_shape = input_shape[-self._num_constants:]  # pylint: disable=E1130
    ...
    def get_initial_state(self, inputs):
        print("GatedConvRNN2D get_initial_state occur!")
        # (samples, timesteps, rows, cols, filters)
        initial_state = K.zeros_like(inputs)
        # (samples, rows, cols, filters)
        initial_state = K.sum(initial_state, axis=1)
        depthwise_shape = list(self.cell.depthwise_kernel_shape)
        pointwise_shape = list(self.cell.pointwise_kernel_shape)
        initial_state = self.cell.input_conv(initial_state, K.zeros(tuple(depthwise_shape)),
                                             K.zeros(tuple(pointwise_shape)), padding=self.cell.padding)
    ...
    def __call__(self, inputs, initial_state=None, constants=None, **kwargs):
        print("GatedConvRNN2D __call__ occur!")
        inputs, initial_state, constants = _standardize_args(
            inputs, initial_state, constants, self._num_constants)

        if initial_state is None and constants is None:
            return super(GatedConvRNN2D, self).__call__(inputs, **kwargs)
        ...
        if K.is_keras_tensor(additional_inputs[0]):
            ...
            original_input_spec = self.input_spec
            self.input_spec = full_input_spec
            output = super(GatedConvRNN2D, self).__call__(full_input, **kwargs)
            self.input_spec = original_input_spec
            return output
        else:
            return super(GatedConvRNN2D, self).__call__(inputs, **kwargs)
    ...
    def call(self,
             ...
             initial_state=None,
             constants=None):
        print("GatedConvRNN2D call occur!")
        # note that the .build() method of subclasses MUST define
        # self.input_spec and self.state_spec with complete input shapes.
        if isinstance(inputs, list):
            inputs = inputs[0]
...
class GatedConvLSTM2DCell(Layer):
    """Cell class for the ConvLSTM2D layer.
  ...
  """
    ...
    def __init__(self,
                 ...
                 recurrent_dropout=0.,
                 **kwargs):
        print("GatedConvLSTM2DCell __init__ occur!")
        super(GatedConvLSTM2DCell, self).__init__(**kwargs)
        self.filters = filters
        self.kernel_size = conv_utils.normalize_tuple(kernel_size, 2, 'kernel_size')
        self.strides = conv_utils.normalize_tuple(strides, 2, 'strides')
        self.padding = conv_utils.normalize_padding(padding)
    ...
    def build(self, input_shape):
        print("GatedConvLSTM2DCell build occur!")
        if self.data_format == 'channels_first':
            channel_axis = 1
        else:
            channel_axis = -1
    ...
    def call(self, inputs, states, training=None):
        ...
        if 0 < self.dropout < 1 and self._dropout_mask is None:
            self._dropout_mask = _generate_dropout_mask(
                ...
                training=training,
                count=4)
            print("GatedConvLSTM2DCell call occur!")
        if (0 < self.recurrent_dropout < 1 and
                self._recurrent_dropout_mask is None):
            self._recurrent_dropout_mask = _generate_dropout_mask(
                K.ones_like(states[1]),
                ...
                training=training,
                count=4)
            print("GatedConvLSTM2DCell call occur2!")
        ...
        if 0 < self.dropout < 1:
            ...
            h_c = self.recurrent_conv(h_tm1 * rec_dp_mask[2], self.recurrent_depthwise_kernel,
                                      self.recurrent_pointwise_kernel)
        else:
            x_c = self.input_conv(
                inputs, self.depthwise_kernel, self.pointwise_kernel, self.bias_c, padding=self.padding)
            h_c = self.recurrent_conv(h_tm1, self.recurrent_depthwise_kernel, self.recurrent_pointwise_kernel)
    ...
    def context_gating(self, x, w, rx, rw, b=None, padding='valid'):
        print("GatedConvLSTM2DCell context_gating occur!")
    ...
    def input_conv(self, x, dw, pw, b=None, padding='valid'):
        print("GatedConvLSTM2DCell input_conv occur!")
    ...
    def recurrent_conv(self, x, dw, pw):
        print("GatedConvLSTM2DCell recurrent_conv occur!")
    ...
    def get_config(self):
        ...
        config = {'filters': self.filters,
                  ...
                  'dropout': self.dropout,
                  'recurrent_dropout': self.recurrent_dropout}
        base_config = super(GatedConvLSTM2DCell, self).get_config()
        return dict(list(base_config.items()) + list(config.items()))


@tf_export('keras.layers.ConvLSTM2D')
class GatedConvLSTM2D(GatedConvRNN2D):
    """Convolutional LSTM.
  ...
  """
    ...
    def __init__(self,
                 ...
                 recurrent_dropout=0.,
                 **kwargs):
        print("GatedConvLSTM2D __init__ occur!")
        cell = GatedConvLSTM2DCell(filters=filters,
                                   kernel_size=kernel_size,
                                   strides=strides,
                                   padding=padding,
                                   data_format=data_format,
                                   ...
                                   dropout=dropout,
                                   recurrent_dropout=recurrent_dropout)
        super(GatedConvLSTM2D, self).__init__(cell,
                                              return_sequences=return_sequences,
                                              go_backwards=go_backwards,
                                              stateful=stateful,
                                              **kwargs)
    ...
    def call(self, inputs, mask=None, training=None, initial_state=None):
        print("GatedConvLSTM2D call occur!")
        return super(GatedConvLSTM2D, self).call(inputs,
                                                 mask=mask,
                                                 training=training,
                                                 initial_state=initial_state)
